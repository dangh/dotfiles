...
def plugin_loaded():
	...
	class Pref:
		def load(self):
			...
			Pref.draw_outlined                                       = sublime.DRAW_SOLID_UNDERLINE ^ sublime.DRAW_NO_FILL ^ sublime.DRAW_NO_OUTLINE
...
class WordHighlightListener(sublime_plugin.EventListener):
	...
	def highlight_occurences(self, view):
		...
		for sel in view.sel():
			if sel.empty():
				if Pref.highlight_when_selection_is_empty:
	...
	def find_regions(self, view, regions, string, limited_size):
		...
		for region in regions:
			if view.match_selector(region.begin(), "keyword"):
				continue
			if view.match_selector(region.begin(), "storage"):
				continue
